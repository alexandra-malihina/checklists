    ...
    <div class="accordion-item">
        <h2 class="accordion-header" :id="check_block_id" @click="getItems">
            <button
                ...
                :data-bs-target="'#collapse_' + check_block_id"
                aria-expanded="false"
                :aria-controls="'collapse_' + check_block_id"
            >
                {{ check_list_info.name }}
                <span
                    class="ms-3 badge bg-success"
                    v-if="check_list_info.is_done"
                >
                    Выполнен</span
                >
        ...
        <div
            :id="'collapse_' + check_block_id"
            class="accordion-collapse collapse"
            aria-labelledby="headingOne"
            :data-bs-parent="'#' + check_block_id"
        >
    ...
    methods: {
        ...
        deleteCheckList() {
            let is_delete = confirm(
                'Вы точно хотите удалить чек-лист "' +
                    this.check_list_info.name +
                    '"?'
            );
            if (!is_delete) {
                return false;
            ...
            axios
                .delete("/api/check-lists/" + this.check_list_info.id)
                .then((r) => {
        ...
        getItems() {
            ...
            this.$emit("set-loading", true);
            axios
                .get("/api/check-lists/" + this.check_list_info.id + "/items")
                .then((r) => {
                    this.$emit("set-loading", false);
                    this.items = r.data;
        ...
        addNewCheckListItem() {
            ...
            axios
                .post(
                    "/api/check-lists/" + this.check_list_info.id + "/items",
                    {
                        name: this.new_item,
                    }
                )
