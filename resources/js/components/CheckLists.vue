<template>
    <div>
        ...
        <div class="container my-5 bg-white p-3">
            <div class="row col col-12 col-md-6 mb-3 mx-auto">
				...
				<div class=" form-group input-group">
					<input
						...
						class="form-control"
					/>
					<div class=" btn btn-outline-success input-group-append " :class="{disabled:!new_checklist}" @click="addNewCheckList">
						Добавить
					</div>
				</div>
            </div>
			<div class="row mb-3 ">
				...
				{{ user.current_check_lists_count }} / {{ user.max_check_lists_count }}
				</h2>
			</div>
			<div class=" accordion col col-12 col-md-6 mx-auto">
				<check-list v-for="checklist in checklists" v-bind:key="checklist.id" v-bind:check_list_info="checklist" @set-loading="setLoading" @set-message="setMessage"></check-list>
			</div>
        </div>
    </div>
</template>
